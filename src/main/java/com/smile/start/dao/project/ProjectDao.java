package com.smile.start.dao.project;
...
public interface ProjectDao {
     ...
     * @return
     */
    @Insert("insert into factoring_project (project_id,kind,project_name,person,progress,model,step,create_time) "
            + "values (#{projectId},#{kind},#{projectName},#{user.id},#{progress},#{model},#{step},#{createTime})")
    ...
    @Select("<script>" + "select * from factoring_project p where \n"
            + "not EXISTS (select 1 from project_meeting pm inner join meeting m where m.kind = 'DIRECTORS' and pm.project_id = p.id and m.id = pm.meeting_id)"
            + "<if test = 'user!=null'>and person = #{user.id} </if> " + "</script>")
    ...
    @Results(id = "queryFactoringProjectMap", value = { @Result(id = true, column = "id", property = "id"), @Result(column = "person", property = "user.id"),
                                                        ...
                                                        @Result(column = "loan_audit_pass_time", property = "detail.loanAuditPassTime") })
