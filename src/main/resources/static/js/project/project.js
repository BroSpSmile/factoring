...
var vue = new Vue({
    ...
    created: function () {
        ...
        this.initDate();
        this.formInline.step=this.step;
        this.query();
    },
    filters: {
    ...
    methods: {
         ...
         */
        addProject: function () {
            window.location.href = "factoring";
        ...
        changeStep: function (name) {
            this.formInline.step = name;
            this.search();
        },
        ...
        toMenu: function (menu, menuName, projectId) {
            if (projectId) {
                window.location.href = menu + "?id=" + projectId;
            } else {
    ...
    render: (h, param) => {
        return h('div', [
            param.row.step == "0" ?h('Button', {
                ...
                on: {
                    click: () => {
                        vue.toMenu('factoring','编辑', param.row.id);
                    }
                }
            }, '编辑') : h('span'),
            param.row.step == '1'||param.row.step == '4'?h('Button', {
                props: {
                    size: "small",
                    type: "info",
                    ghost: true
                ...
                on: {
                    click: () => {
                        vue.toMenu('factoring','查看', param.row.id);
                    }
                }
            }, '查看') : h('span'),
            param.row.step == '2'?h('Button', {
                props: {
                    size: "small",
                    type: "success",
                    ghost: true
                ...
                on: {
                    click: () => {
                        vue.toMenu('factoringLoan','放款录入', param.row.id);
                    }
                }
            }, '放款录入') : h('span'),
            param.row.step == '3'?h('Button', {
                props: {
                    size: "small",
                    type: "success",
                    ghost: true
                ...
                on: {
                    click: () => {
                        vue.toMenu('afterLoan','贷后管理', param.row.id);
                    }
                }
            }, '贷后管理') : h('span')]);
        // return h('div', [
